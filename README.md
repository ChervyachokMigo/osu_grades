...
- Копирование шаблон файла конфигурации: запустить `1 set_config.bat` 
  > Батник создат папку `data` и скопирует пример конфигурации из `misc/config-sample.json`. затем переименует его в `config.json` и запустит для редактирования
- Редактирование `config.json` (в любом текстовом редакторе, например [Sublime text](https://www.sublimetext.com/download) или [Notepad++](https://notepad-plus-plus.org/downloads/))
  ...
  - **Необязательные параметры** в *config.json*, которые потребуются не сразу, а только для определенных функций либо косметические
