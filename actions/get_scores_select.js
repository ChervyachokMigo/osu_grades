...
const get_scores_v1_async = require('./get_scores_v1_async');
const get_scores_v2 = require('./get_scores_v2');
const get_scores_v2_async = require('./get_scores_v2_async');
...
const actions_async = {
	1: get_scores_v1_async.action,
	2: get_scores_v2_async.action
}
...
module.exports = {
	...
	action: async( args ) => {
		await users.users_variants( 'checkboxes', true, async ( selected ) => {
			...
			switch( await input.select( 'Request type', ['Sync', 'Async']) ){
				case 'Sync':
                ...
                case 'Async':
					await actions_async[selected.score_mode]({ 
						userid: selected.userid, 
						gamemode: selected.gamemode, 
						continue_md5: args.continue_md5 
					});
